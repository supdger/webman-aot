...
| v0.1.1 变更候选（版本号升级前） | [公开验收运行记录](https://github.com/supdger/webman-aot/actions/runs/36216983514) | Windows Server 2022 上从源码两次构包，134 个包内文件一致；安装时自动校验 130 个 payload 文件，`doctor --repair` 校验 21/21 组件，环境检查健康，中立 Webman 2.2.4 项目编译及 `verify` 通过，随后从项目目录卸载成功。此运行测试的是 `0.1.0` 版本号，尚不是最终发布包 |
